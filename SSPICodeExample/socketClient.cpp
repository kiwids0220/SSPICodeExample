...
//  the server program.
const wchar_t* TargetName = L"corp\\Labadmin";
const wchar_t* ServerName = L"CLIENT2.CORP.CONTOSO.COM";
char* ServerName_char = (char*)"CLIENT2.CORP.CONTOSO.COM";
    ...
    struct _SecHandle* hcText)
{
    ...
    ulAddress = inet_addr(ServerName_char);

    if (INADDR_NONE == ulAddress)
    {
        pHost = gethostbyname(ServerName_char);
        if (NULL == pHost)
        {
    ...
    struct _SecHandle* hcText)
{
    ...
    // Kiwids: OutBuffDesc is a wrapper that can contain arrays of SecBuffer https://learn.microsoft.com/en-us/previous-versions/windows/embedded/ms926434(v=msdn.10)
    // in this case. the version is 0, and countBuffers = 1 meaning there's only one SecBuffer struct inside of the array.
    OutBuffDesc.ulVersion = 0;
    OutBuffDesc.cBuffers = 1;
    OutBuffDesc.pBuffers = &OutSecBuff;
    // Kiwids: OutSecBuffer is a buffer that an application allocates to pass to a security package https://learn.microsoft.com/en-us/previous-versions/windows/embedded/ms926433(v=msdn.10)
    ...
    // this is the first ISC, so no input buffer yet
    else
    {
        ss = InitializeSecurityContext(
            hCred,
